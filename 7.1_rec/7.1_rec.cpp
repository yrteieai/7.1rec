...
void SortMatrix(int** matrix, const int rowCount, const int colCount, int i = 0, int j = 0) {
    if (i >= colCount - 1) {
        return;  // База рекурсії: коли всі стовпці оброблені
    }

    if (j < colCount - i - 1) {
        // Порівняння для першого рядка (сортування за зростанням)
        if (matrix[0][j] > matrix[0][j + 1]) {
            SwapColumns(matrix, j, j + 1, rowCount);  // Переставляємо стовпці
        }
        // Якщо в першому рядку елементи однакові, переходимо до другого рядка
        else if (matrix[0][j] == matrix[0][j + 1]) {
            // Порівняння для другого рядка (сортування за спаданням)
            if (matrix[1][j] < matrix[1][j + 1]) {
                SwapColumns(matrix, j, j + 1, rowCount);  // Переставляємо стовпці
            }
            // Якщо в другому рядку також однакові, перевіряємо третій рядок
            else if (matrix[1][j] == matrix[1][j + 1]) {
                // Порівняння третього рядка (сортування за спаданням)
                if (matrix[2][j] < matrix[2][j + 1]) {
                    SwapColumns(matrix, j, j + 1, rowCount);  // Переставляємо стовпці
                }
            }
        }
        // Переходимо до наступної пари стовпців у поточній ітерації
